...
// Global game state singleton. Carries world flags (waves cleared, spawners
// disabled, events fired) and owns JSON save/load. This is the Unity port of
// the NoxDev template ABI's "game_manager" autoload: anything that persists
...
//
// Shared, byte-for-concept, with the top-down-action-unity port: the same
// ISaveable / IDamageable contracts and the same save-document shape, so a
// bullet-hell save round-trips through the identical machinery.
...
namespace NoxDev.BulletHell
{
    ...
    /// <summary>
    /// The take_hit(damage, from) damage contract shared by everything the
    /// bullet system can strike (the player ship today; future enemies and
    /// destructibles tomorrow). Mirrors the Godot template's take_hit().
    /// </summary>
    ...
    public sealed class GameManager : MonoBehaviour, ISaveable
    {
        ...
        /// contain one, so scripts can rely on it unconditionally (the Unity
        /// analogue of the "GameManager" autoload).
        /// </summary>
        ...
        /// <summary>World flags: waves cleared, spawners disabled, events fired.</summary>
        ...
        // {"game_manager": {"flags": {...}}, "player": {"position": ..., "lives": ...}}
        // -------------------------------------------------------------------
