...
## AntWorld — the pure colony-sim engine (scripts/ant_world.gd) — and exposes it
...
## save_system persists the ENTIRE world (terrain + both colonies + pheromone
## fields + RNG) — a full replayable snapshot — alongside flags.

signal world_reset  ## a fresh world was created (the view rebinds + repaints)

const GRID_W := 56             ## the colony map is GRID_W × GRID_H cells…
const GRID_H := 40
