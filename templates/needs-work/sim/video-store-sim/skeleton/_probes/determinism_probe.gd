...
## DETERMINISM probe: the same seed reproduces a BYTE-IDENTICAL store (identical
## FNV-1a checksum over quantised state), both mid-run and at the end; a DIFFERENT
## seed diverges; and world/setup state is itself seeded. The printed CANON= value is
## a fixed-seed full-run checksum that is STABLE ACROSS SEPARATE PROCESSES — the build
...
func _run_n(seed_value: int, n: int) -> int:
	var e: VideoStoreEngine = VideoStoreEngine.new()
	e.setup(seed_value, {"policy": "aggressive"})
	for _i in n:
		if e.outcome != VideoStoreEngine.ONGOING:
			break
		e.auto_play_step()
	return e.state_checksum()
...
func _ready() -> void:
	...
	var m1: int = _run_n(4242, 40)
	var m2: int = _run_n(4242, 40)
	if m1 != m2:
		fails += 1
	...
	var s1: VideoStoreEngine = VideoStoreEngine.new()
	s1.setup(111)
	for _i in 30:
		s1.auto_play_step()
	var s2: VideoStoreEngine = VideoStoreEngine.new()
	s2.setup(222)
	for _i in 30:
		s2.auto_play_step()
