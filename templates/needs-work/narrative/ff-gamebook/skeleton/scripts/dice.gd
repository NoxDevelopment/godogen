...
## your luck also spends a point of LUCK afterwards, win or lose. Dialogue
## routes tests through SessionState (the multiplayer sync point), which
## awaits this layer:
##
##     do SessionState.roll("skill")
##     if Dice.last_success
##         => passage_7
##
##     do SessionState.roll_luck()
##
## The dice-tray popup is awaited as part of the flow; `show_popup = false`
## resolves tests instantly (boot probe, and later the ENet replay path).
...
## When false, test() resolves without the dice-tray popup (probe/headless).
var show_popup := true
...
## The full test flow: roll, tumble the dice tray, wait for the player to
## confirm. Await-able; returns the success bool. Prefer calling it through
## SessionState.roll() so the result lands in the session's roll_log.
