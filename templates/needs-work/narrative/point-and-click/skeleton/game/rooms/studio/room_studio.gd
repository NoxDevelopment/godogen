...
const Data := preload('room_studio_state.gd')

var state: Data = load("res://game/rooms/studio/room_studio.tres")
...
func _on_room_entered() -> void:
	# Start the player on the walkable floor strip (Popochiu drops the PC at the
	# room center by default, which sits on the wall in this blockout).
	if is_instance_valid(C.player):
		C.player.position = Vector2(160, 160)
