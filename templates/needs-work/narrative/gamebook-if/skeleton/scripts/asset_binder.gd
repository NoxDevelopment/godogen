...
## Asset binder (autoload "AssetBinder"): reads res://assets.manifest.json — the
## contract between this project and the Studio asset board. Every art/audio
## surface in the game is a named SLOT; the manifest records, per slot, HOW it
## gets filled (policy/workflow/stylePack/promptTemplate) and WHAT currently
## fills it (file + provenance). Scene code never hardcodes asset paths — it asks
## the binder:
##
##     AssetBinder.get_texture("illustration/crypt_gate")   # null until generated
##     AssetBinder.placeholder_color("ui/dice_tray")        # deterministic tint
##
## `file: null` means "not generated yet" — callers show a placeholder. The
## Studio asset board runs each slot's workflow, writes the asset into the
## project and updates the slot's `file` + `provenance`; the next boot binds the
## real art with zero code changes. This is the SAME binder the ff-gamebook
## template ships — the illustrated-page identity is preserved. Passages WITHOUT
## a declared slot fall back to a titled placeholder (see illustration_plate.gd),
## so a data-driven, many-scenario gamebook needs no per-passage manifest churn.
...
func reload() -> void:
	...
	if not FileAccess.file_exists(MANIFEST_PATH):
		# No manifest is fine: every plate falls back to a titled placeholder.
		return
...
## {"illustration": N, "ui": M, "audio": K} — used by the boot probe.
...
## placeholder). Falls back to loading the image from disk for files the Studio
## board dropped in after the last editor import.
...
## Placeholder tint for an unfilled slot: the slot's explicit `placeholderColor`
## if declared, else a deterministic muted ink tone derived from the slot id
## (stable across boots, distinct per slot).
