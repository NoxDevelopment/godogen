...
##   • NEW to open the 3D designer on a blank canvas,
...
## single hand-off obby3d.gd reads — the SAME mechanism the editor's Test uses, so
...
## so the ui-theme / settings_system drop-ins re-skin it with no code change. It
## is course_select.gd (the 2D template) with CourseData→CourseData3D and the
## editor/play scenes pointed at the 3D versions.
##
## NOTE: every `lib.<method>()` result is stored in an EXPLICITLY typed local (lib
## is a base Node from get_node_or_null, so `:=` cannot infer the return type —
## an un-annotated `var x := lib.load_course(...)` fails to parse as a main scene).

const EDITOR_SCENE := "res://scenes/course_editor3d.tscn"
...
func _on_play() -> void:
	...
	if e.is_empty():
		...
		return
	var lib := get_node_or_null("/root/CourseLibrary")
	var course: CourseData3D = lib.load_course(str(e["id"]))
	if course == null:
...
func _on_import() -> void:
	...
	if src.is_empty():
		...
		return
	var lib := get_node_or_null("/root/CourseLibrary")
	var course: CourseData3D = lib.import_course(src)
	if course == null:
