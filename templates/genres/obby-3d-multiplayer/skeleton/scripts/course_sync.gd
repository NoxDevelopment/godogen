...
## The 3D obby's course-sync layer — the ONE piece of game code that makes an
## online race play the HOST's chosen course on every peer. It sits on the level
## root as the fixed-name child "CourseSync" (so its RPC path is identical on all
## peers), right alongside nox_netcode's NetSpawner/NetEvents, and never touches
## the addon: it is host-authoritative in exactly the same spirit (clients
## REQUEST, the host BROADCASTS the single source of truth).
##
## Wire format is the same CourseData3D JSON used for files — for a built-in the
## host sends just its id (compact + guaranteed present on every client), for a
## custom/imported course it sends the full CourseData3D JSON so clients without
## the file still build the identical geometry. There is no separate net schema.
##
## Offline (Net.active == false) this node is completely inert: obby3d.gd only
## ever calls into it inside its `_net_active()` branch, so the single-player path
## is byte-identical to before. It is course_sync.gd (the 2D template) with
## CourseData→CourseData3D; the RPC contract is identical.

## The host published a course (call_local → fires on the host too) OR a client
## received the host's reply. Carries the resolved CourseData3D to build.
signal course_ready(course: CourseData3D)

## The last course this node resolved (host: what it published; client: what it
## received). Null until the first publish/receive.
var active_course: CourseData3D = null
...
## on a client.
func publish(course: CourseData3D, builtin_id: String = "") -> void:
...
## is a built-in listing id) or "json" (payload is CourseData3D JSON).
...
func _resolve(kind: String, payload: String) -> CourseData3D:
	if kind == "id":
		var lib := get_node_or_null("/root/CourseLibrary")
		if lib != null:
			var c: CourseData3D = lib.load_course(payload)
			if c != null:
				return c
		# Fall back to the built-in factory directly if the autoload is absent.
		if payload == CourseLibrary.BUILTIN_PREFIX + "vertigo_gauntlet":
			return CourseLibrary.vertigo_gauntlet()
		return CourseLibrary.skyward_steps()
	return CourseData3D.from_json(payload)
