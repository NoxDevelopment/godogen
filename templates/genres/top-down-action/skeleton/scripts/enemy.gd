...
## Chaser enemy: follows the player across the arena's NavigationRegion2D via
## a NavigationAgent2D, repathing on a short timer, and deals contact damage
## with a cooldown when it reaches the player. Shootable (take_hit contract).
...
## True once the agent has a live path towards the player (used by the arena's
