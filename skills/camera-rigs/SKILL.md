...
Camera motion is **visible output** — run the scene and **watch it**: confirm smooth follow with no jitter, shake that decays cleanly, and correct pitch/bounds behavior. Check at 30 / 60 / 144 Hz (motion smoothing is framerate-sensitive). For a template, feel must meet `skills/parity-build/STANDARDS.md` → "Game feel".
